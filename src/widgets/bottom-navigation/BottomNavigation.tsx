...
const BottomNavigation = () => {
  ...
  return (
    <>
      <nav className="fixed bottom-4 right-0 left-0 flex justify-start items-start w-[330px] m-auto z-40 backdrop-blur-md bg-white/0 shadow-2xl border border-l-0 border-r-0 border-white/50 rounded-full p-1 space-x-[-4px] h-14">
        {navItems.map(item => {
          ...
          return (
            <Link
              ...
              className={`flex justify-center items-center m-auto flex-grow h-full relative rounded-full transition-all duration-200 ease-in-out ${
                isActive &&
                'border border-l-0 border-r-0 border-white/40 backdrop-blur-md bg-white/10'
              }`}
              href={item.href}
            >
              <p
